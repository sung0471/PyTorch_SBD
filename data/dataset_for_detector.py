...
detector_name = 'detector_L=32.txt'
...
if make_detector_from_groundTruth_and_deepSBD_new:
    ...
    with open(deepSBD_new, 'rt', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            destination.write('{} {} {} {}'.format(video_name, start, label, dir_name))
            ...
            for gt_s, gt_e in gt[dir_name][video_name][transition_type[label]]:
                ...
                if check:
                    destination.write('{} {}'.format(gt_s, gt_e))
            count[check] += 1
            if check:
                check = not check
            else:
    ...
    json.dump(no_gt, open('no_gt_L=32.json', 'wt', encoding='utf-8'), indent=2)
